...
if (($checkAge > '59') || ($checkAge > '0' && $checkAge < '7') || ($checkAge < '1')){
    //edit end
    $query = "INSERT INTO residents (fname, mname, lname, bdate, age, gender, guardian, address, status, contactNum, contactNum2, mother, father, moccupation, foccupation, disability, created_by, updatedBy) VALUES ('$fname', '$mname', '$lname', '$birthday', '-1','$gender', '$guardian', '$address', '$status', '$mobile1', '$mobile2', '$mother', '$father', '$motherOccupation', '$fatherOccupation', '$disability', '".$_SESSION['username']."', '')";
        $result = mysqli_query($db,$query) or die(mysqli_error($db));
        if($result){
        $query = "UPDATE residents SET age = TIMESTAMPDIFF(year, bdate, createdAt) WHERE age = '-1'";
