...
if (isset($_REQUEST['event_name'], $_REQUEST['event_cate'], $_REQUEST['event_loc'], $_REQUEST['event_caps'], $_REQUEST['event_part'], $_REQUEST['event_start'], $_REQUEST['event_end'],  $_REQUEST['coor_name'], $_REQUEST['coor_num'])){
    ...
    $query = "INSERT INTO services (servName, servCate, servPlace, servParticipant, servCapacity, servStart, servEnd, coorName, coorNum, createdBy, updatedBy)  VALUES ('$event_name', '$event_cate', '$event_loc', '$event_part', '$event_caps', '$event_start', '$event_end','$coor_name', '$coor_num', '".$_SESSION['username']."','')";
